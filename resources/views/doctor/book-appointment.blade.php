...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/doctor-book-appointment.css') }}">
...
<div class="notifications">
    @if(session('success'))
        <script>createToast('success', 'fa-solid fa-circle-check', 'Success', '{{ session('success') }}');</script>
    @endif
    @if(session('error'))
        <script>createToast('error', 'fa-solid fa-circle-exclamation', 'Error', '{{ session('error') }}');</script>
    @endif
</div>
...
<div class="container">
    ...
    <div class="content">
        ...
        <form method="POST" action="{{ route('doctor.book-appointment.store') }}" class="appointment-form" id="appointment-form" enctype="multipart/form-data">
            @csrf
            ...
            <div class="select-date-time">
                <div class="req-appointment-calendar">
                    @include('patient.request-appointment-calendar', ['appointmentDates' => $appointmentDates])
                </div>
...
<script>
    ...
    document.getElementById('patient-mykad').addEventListener('blur', function () {
        ...
        if (mykad) {
            fetch("{{ route('doctor.fetch-patient') }}?mykad=" + encodeURIComponent(mykad))
    ...
    function loadAvailableSlots(sectionId, dayName) {
        fetch(`{{ route('doctor.available-slots') }}?section_id=${sectionId}&day=${dayName}`)
            .then(res => res.json())
            .then(slots => {
                let amHTML = '', pmHTML = '';
